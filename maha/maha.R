...
# 🚩 图2. N种疾病与4个diet的关联
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure 2")

diet_cols <- c(MAHA="#F26D60", DASH="#16B9C0", MIND="#B97AF7", MEDI="#7CAE00")
diet_levels <- c("MAHA","DASH","MIND","MEDI"); outcome_levels <- rev(unname(dx.lst[Y.inc]))

dat1 <- dat; for (x in diet.inc.pts) dat1[[x]] <- as.numeric(scale(dat1[[x]]))
assoc.res <- assoc_reg(dat1, diet.inc.pts, covs, names(dx.lst), type="t2e") %>%
	mutate(Outcome=factor(unname(dx.lst[Outcome]), levels=outcome_levels),
		Diet=factor(unname(diet.lst[gsub("^diet\\.|\\.(pts|q5|qt)$","",Exposure)]), levels=diet_levels)) %>%
	filter(Outcome %in% outcome_levels, Diet %in% diet_levels)

shift4 <- c(MAHA=-0.27, DASH=-0.09, MIND=0.09, MEDI=0.27)
pLdat <- assoc.res %>% arrange(Outcome, Diet) %>% mutate(y0=as.numeric(Outcome), y=y0+unname(shift4[as.character(Diet)]), lab=sprintf("%.2f (%.2f, %.2f)", estimate, conf.low, conf.high))
xlimL <- range(c(1, pLdat$conf.low, pLdat$conf.high), na.rm=TRUE); xtextL <- max(pLdat$conf.high, na.rm=TRUE)+0.05

p_left <- ggplot(pLdat, aes(color=Diet)) +
	...
	geom_text(aes(x=xtextL, y=y, label=lab, color=Diet), hjust=0, size=4.2, show.legend=FALSE) +
	scale_color_manual(values=diet_cols, breaks=diet_levels) +
	scale_y_continuous(breaks=seq_along(outcome_levels), labels=outcome_levels, expand=expansion(add=c(0.5,0.5))) +
	coord_cartesian(xlim=c(xlimL[1]-0.03, xtextL+0.18), clip="off") +
	labs(title="A. Main associations", x="Hazard ratio", y=NULL, color=NULL) +
	theme_classic(base_size=16) +
	theme(plot.title=element_text(face="bold", hjust=0, size=16), legend.position="none",
		axis.text.y=element_text(face="bold"), axis.title.x=element_text(face="bold"),
		plot.margin=margin(5.5,70,5.5,5.5))

diet.pairs <- c(dash="DASH", medi24="MEDI", mind="MIND"); shift3 <- c(DASH=-0.22, MEDI=0, MIND=0.22)
fit_disc_one <- function(dat, Y, trad_nm, trad_lab, covs){
	trad_var <- paste0("diet.", trad_nm, ".hml")
	d0 <- dat %>% transmute(time=.data[[paste0(Y,".t2e")]], event=.data[[paste0(Y,".Yt2e")]], across(all_of(covs)), trad=.data[[trad_var]], maha=diet.maha.hml) %>%
		filter(trad %in% c("low","high"), maha %in% c("low","high")) %>% drop_na()
	g_hl <- paste0(trad_lab," high + MAHA low"); g_lh <- paste0(trad_lab," low + MAHA high")
	d0 <- d0 %>% mutate(discord=factor(case_when(trad=="high" & maha=="low" ~ g_hl, trad=="low" & maha=="high" ~ g_lh, TRUE ~ NA_character_), levels=c(g_hl,g_lh)))
	coxph(Surv(time,event) ~ discord + ., data=d0 %>% filter(!is.na(discord)) %>% select(time,event,all_of(covs),discord)) %>%
		broom::tidy(exponentiate=TRUE, conf.int=TRUE) %>% filter(grepl("^discord", term)) %>%
		transmute(Pattern=trad_lab, Outcome=factor(unname(dx.lst[Y]), levels=outcome_levels), contrast=paste0(g_lh," vs ",g_hl), estimate, conf.low, conf.high, p.value)
}
tab.discord <- purrr::imap_dfr(diet.pairs, ~ purrr::map_dfr(Y.inc, fit_disc_one, dat=dat, trad_nm=.y, trad_lab=.x, covs=covs)) %>%
	mutate(Pattern=factor(Pattern, levels=c("DASH","MEDI","MIND")))
pRdat <- tab.discord %>% arrange(Outcome, Pattern) %>% mutate(y0=as.numeric(Outcome), y=y0+unname(shift3[as.character(Pattern)]))
xlimR <- range(c(1, pRdat$conf.low, pRdat$conf.high), na.rm=TRUE)

p_right <- ggplot(pRdat, aes(color=Pattern)) +
	geom_vline(xintercept=1, linetype="dashed", color="grey55", linewidth=0.8) +
	geom_segment(aes(x=conf.low, xend=conf.high, y=y, yend=y), linewidth=1) +
	geom_point(aes(x=estimate, y=y), size=3.2) +
	scale_color_manual(values=diet_cols, breaks=diet_levels) +
	scale_y_continuous(breaks=seq_along(outcome_levels), labels=rep("", length(outcome_levels)), expand=expansion(add=c(0.5,0.5))) +
	coord_cartesian(xlim=c(xlimR[1]-0.03, xlimR[2]+0.05), clip="off") +
	labs(title="B. Discordant contrasts", x="Hazard ratio", y=NULL, color=NULL) +
	theme_classic(base_size=16) +
	theme(plot.title=element_text(face="bold", hjust=0, size=16), legend.position="none",
		axis.text.y=element_blank(), axis.ticks.y=element_blank(), axis.title.x=element_text(face="bold"))

fit_aic_one <- function(dat, Y, trad_nm, trad_lab, covs){
	d0 <- dat %>% transmute(time=.data[[paste0(Y,".t2e")]], event=.data[[paste0(Y,".Yt2e")]], across(all_of(covs)), maha=as.numeric(scale(diet.maha.pts)), trad=as.numeric(scale(.data[[paste0("diet.",trad_nm,".pts")]]))) %>% drop_na()
	...
	tibble(Pattern=trad_lab, Outcome=unname(dx.lst[Y]), Base=a0, MAHA=am, Traditional=at, Both=ab, add_MAHA_given_trad=at-ab, add_trad_given_MAHA=am-ab,
		maha_only_information=pmax(at-ab,0), traditional_only_information=pmax(am-ab,0), net_traditional_gain=(am-ab)-(at-ab))
}
tab.aic <- purrr::imap_dfr(diet.pairs, ~ purrr::map_dfr(Y.inc, fit_aic_one, dat=dat, trad_nm=.y, trad_lab=.x, covs=covs))

write_xlsx(list(
	Fig2_assoc=assoc.res %>% transmute(Outcome=as.character(Outcome), Diet=as.character(Diet), HR=estimate, LCI=conf.low, UCI=conf.high, P=p.value, N_total, N_event),
	Fig2_discordant=tab.discord %>% transmute(Outcome=as.character(Outcome), Pattern=as.character(Pattern), contrast, HR=estimate, LCI=conf.low, UCI=conf.high, P=p.value),
	Fig2_aic=tab.aic), "Fig2.data.xlsx")

leg.df <- data.frame(Diet=factor(diet_levels, levels=diet_levels))
g_leg <- ggplot(leg.df, aes(1,1,color=Diet)) + geom_point(size=4) +
	scale_color_manual(name=NULL, values=diet_cols, breaks=diet_levels) +
	...
	theme(legend.position="bottom", legend.justification="center", legend.title=element_blank(),
		legend.text=element_text(size=14, face="bold"), legend.margin=margin(0,0,0,0), legend.box.margin=margin(0,0,0,0))
leg <- cowplot::get_legend(g_leg)

prow <- cowplot::plot_grid(p_left, p_right, nrow=1, rel_widths=c(1.35,0.75), align="h", axis="tb")
Fig2 <- cowplot::plot_grid(prow, cowplot::plot_grid(NULL, leg, NULL, nrow=1, rel_widths=c(0.10,0.80,0.10)), ncol=1, rel_heights=c(1,0.10))

save_plot(Fig2, "Fig2.png", width=12, height=7.8, dpi=320)
print(assoc.res); print(tab.discord); print(tab.aic)


#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 🚩 图3. DASH 🤺 MAHA 📊🧱
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure 3: 10-year risk")
...
save_plot(Fig3, "Fig3.png", width = 10.8, height = 11.0, dpi = 320)
fig3_tab <- dat1 %>% count(diet.dash.3c, diet.maha.3c, name = "N") %>% mutate(Outcome = dx.lst[Y])
save_xlsx1(fig3_tab, "Fig3.out.xlsx")
print(fig3_tab)
...
# 🚩 图S1. 不同diet评分之间的相关性🍉
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure S1： diet correlation")
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure S2：Head-to-head comparison of PheWAS")
...
# 🚩 图S3. 联合打分✝年风险 🔗
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure S3：10-year joint DASH and MAHA categories")
...
# 🚩 图S5. 🗺 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
step_header("Figure S5：geographic distribution")
