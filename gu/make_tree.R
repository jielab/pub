...
if (!keep_existing) unlink(list.files(out0, pattern = "^s8_tree_.*\\.png$", full.names = TRUE), force = TRUE)
...
if (write_pptx && !keep_existing && file.exists(ppt_full)) file.remove(ppt_full)
if (write_pptx && !keep_existing && file.exists(ppt_main)) file.remove(ppt_main)
...
highlight_node <- function(tr, meta) {
  ...
  candidates <- function(all_arch = TRUE) {
    rbindlist(lapply((nt + 1):(nt + tr$Nnode), function(node) {
      ...
      ok_arch <- if (all_arch) isTRUE(all(arch %in% tips)) else isTRUE(any(tips %in% arch))
      ...
      if (length(support) != 1L) support <- NA_real_
      if (!ok_arch || !has_modern || has_ancestral ||
...
for (treef in files) {
  ...
  if (write_pptx) {
    ...
    topptx_safe(p, if (tag == "main") ppt_main else ppt_full,
                ...
                append = keep_existing || n_ppt[tag] > 0L)
  }
