...
merge_all <- function(){
	...
	out_files <- c("hap_match.csv", "site_count.csv", "archaic_match.csv", "region_summary.csv", "risk_core.csv", "inherited_segments.csv",
		"hap_match.tsv", "site_count.tsv", "archaic_match.tsv", "region_summary.tsv", "risk_core.tsv", "inherited_segments.tsv",
		"hap_site_count.tsv", "core_archaic_match.tsv", "core_risk.tsv")
	...
	write_report <- function(dt, stem){
		...
		fwrite(dt, file.path(report0, paste0(stem, ".csv")))
		fwrite(dt, file.path(report0, paste0(stem, ".tsv")), sep = "\t")
	}
	write_report(hap_dt, "hap_match"); write_report(site_dt, "site_count")
	write_report(arch_dt, "archaic_match"); write_report(region_dt, "region_summary"); write_report(core_dt, "risk_core")
	if(nrow(region_dt)){ inherited <- region_dt[!is.na(best_lineage) & nzchar(best_lineage) & nzchar(matched_archaics)]; write_report(inherited, "inherited_segments") }
}
