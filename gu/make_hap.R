...
report0 <- file.path(root, "report")  # ❗这儿修改了：合并后的 summary TSV 统一输出到 report/。
...
merge_all <- function(){
	...
	out_files <- c("hap_match.tsv", "hap_site_count.tsv", "core_archaic_match.tsv", "region_summary.tsv", "core_risk.tsv")
	# ❗这儿修改了：删除旧版根目录 summary 和新版 report summary，防止 rerun 后读到旧结果。
	unlink(c(file.path(root, out_files), file.path(report0, out_files)), force = TRUE)
	if(nrow(hap_dt)) fwrite(hap_dt, file.path(report0, "hap_match.tsv"), sep = "\t"); if(nrow(site_dt)) fwrite(site_dt, file.path(report0, "hap_site_count.tsv"), sep = "\t")
	if(nrow(arch_dt)) fwrite(arch_dt, file.path(report0, "core_archaic_match.tsv"), sep = "\t"); if(nrow(region_dt)) fwrite(region_dt, file.path(report0, "region_summary.tsv"), sep = "\t"); if(nrow(core_dt)) fwrite(core_dt, file.path(report0, "core_risk.tsv"), sep = "\t")
}
