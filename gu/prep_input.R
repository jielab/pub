...
if(!exists("SNP_match")) stop("SNP_match() not found. Please check /mnt/d/scripts/f/phe.f.R")
...
rd_pvar <- function(dirmod, lead){
	...
	unique(rbindlist(out, fill=TRUE)[!is.na(CHR) & !is.na(POS) & nzchar(ID) & ID!='.'])
}
...
align_1kg <- function(z, dirmod, lead0, tr){
	...
	d0 <- merge(z[, .(rid, lead_chr, lead_bp, lead_snp)], p[, .(lead_chr=CHR, lead_bp=POS, lead_snp=ID, pvar_id=ID, pvar_bp=POS, pvar_file)], by=c('lead_chr','lead_bp','lead_snp'), all=FALSE, allow.cartesian=TRUE)
	d0 <- d0[, .(rid, pvar_id, pvar_bp, pvar_file, match_type_1kg='ID_exact', priority=0L)]

	d1 <- merge(z[, .(rid, lead_chr, lead_snp)], p[, .(lead_chr=CHR, lead_snp=ID, pvar_id=ID, pvar_bp=POS, pvar_file)], by=c('lead_chr','lead_snp'), all=FALSE, allow.cartesian=TRUE)
	d1 <- d1[, if(.N == 1) .SD, by=rid][, .(rid, pvar_id, pvar_bp, pvar_file, match_type_1kg='ID_only', priority=1L)]

	m <- SNP_match(z, 'lead_chr', 'lead_bp', 'EA', 'NEA', p, 'CHR', 'POS', 'REF', 'ALT', by=c('CHR','POS','REF','ALT'))
	m <- if(nrow(m)) m[, .(rid=A.rid, pvar_id=B.ID, pvar_bp=B.POS, pvar_file=if('B.pvar_file' %chin% names(m)) B.pvar_file else NA_character_, match_type_1kg=match_type, priority=2L)] else data.table(rid=integer(), pvar_id=character(), pvar_bp=integer(), pvar_file=character(), match_type_1kg=character(), priority=integer())

	h <- rbind(d0, d1, m, fill=TRUE); h <- h[!is.na(pvar_id) & nzchar(pvar_id) & pvar_id!='.']
