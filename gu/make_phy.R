...
target_trait <- if(length(args) >= 1L) args[1] else NA_character_
target_id <- if(length(args) >= 2L) args[2] else NA_character_
report0 <- file.path(root, "report")
hapf <- file.path(report0, "hap_match.tsv")
if(!file.exists(hapf)) hapf <- file.path(root, "hap_match.tsv")  # 兼容旧版输出。
mat0 <- file.path(root, "mat"); phy0 <- file.path(root, "phy")
dir.create(phy0, recursive = TRUE, showWarnings = FALSE)
if(!file.exists(hapf) || file.info(hapf)$size == 0) quit(save = "no", status = 1)
base_set <- c("A", "C", "G", "T")
hap <- fread(hapf)
if(!is.na(target_trait)) hap <- hap[trait == target_trait]
if(!is.na(target_id)) hap <- hap[id == target_id]
if(!nrow(hap)) quit(save = "no", status = 0)
old_root <- if(is.na(target_trait)) phy0 else file.path(phy0, target_trait)
oldf <- list.files(old_root, pattern = "\\.(phy|meta\\.tsv|phy_phyml_tree\\.txt|phy_phyml_stats\\.txt)$", recursive = TRUE, full.names = TRUE)
if(is.na(target_id) && length(oldf)) unlink(oldf, force = TRUE)
