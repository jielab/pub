...
# Archaic reference-only site, e.g. REF=A, ALT=.; retain the information as GT=0/0
x[!is.na(REF_arc) & REF_arc == REF_kg & ALT_arc == ".", GT := "0/0"]

# Archaic variant site already uses the same 1000G REF/ALT definition
x[!is.na(REF_arc) & REF_arc == REF_kg & ALT_arc == ALT_kg, GT := GT_arc]

# Normalize haploid or missing calls if present
x[GT == "0", GT := "0/0"]
x[GT == "1", GT := "1/1"]
x[GT == ".", GT := "./."]
